...
from ..common.config import ParameterName, default_parameter_extract
...
def calculate_line_intersect_to_ratio(
    ...
    for intersect_line_vector in intersect_line_vector_list:
        ...
        if abs(coeff_matrix_det) < Constant.computation_eps:
...
def union_of_segment_on_line(segment_list, distinct=False):
    ...
    if distinct:
        return segment_list
    computation_eps = Constant.computation_eps
