...
from .config import np, Vector, Arrow, ArcArrow, complete_arrow_parameter_set_dict, \
    ParameterName, Keyword, ZOrderConfig, ColorConfig, DataName
...
class NetworkDiagram(CompositeFigure):
    ...
    def __init__(self, mode=ParameterName.normal, **kwargs):
        ...
        (
            ...
            background_range, mito_background_range, tca_cycle_center, tca_cycle_radius) = diagram_layout_generator(
            radius, self.total_width, self.height_to_width_ratio, mode)
        ...
        if mode == DataName.smaller_data_size:
            ...
            mixed_metabolite_basic_config_dict = {
                'pyr': construct_mixed_metabolite_obj(
                    glycolysis_center_dict['pyr_c'], glycolysis_center_dict['pyr_m'], metabolite_radius,
                    orientation=ParameterName.vertical),
                'cit': construct_mixed_metabolite_obj(
                    input_center_dict['cit_c'], tca_center_dict['cit_m'], metabolite_radius,
                    orientation=ParameterName.horizontal),
                'glu': construct_mixed_metabolite_obj(
                    input_center_dict['glu_c'], tca_center_dict['akg_m'], metabolite_radius,
                    orientation=ParameterName.vertical),
            }
